...
/*
[참고] 어노테이션(annotation)이란? 소스 코드에 추가해 사용하는 메타 데이터의 일종. 메타
데이터는 프로그램에서 처리해야 할 데이터가 아니라 컴파일 및 실행 과정에서 코드를 어떻게
처리해야 할지 알려 주는 추가 정보. 자바에서 어노테이션은 앞에 @기호를 붙여 사용.
 */
// 스프링 프레임워크에게 이 클래스는 특별한 클래스임을 알려줌
// 자바빈으로 등록한 후 메모리에 미리 new를 해서 업로드 함.
// console창에 썰을 풀어보자
@Slf4j
@Controller
public class FirstController {
    ...
    public ModelAndView niceToMeetYou() {
        ...
        // model 객체가 "석현" 값을 "username"속성에 연결해 웹 브라우저로 보냄
        mav.addObject("username","석현");
        mav.addObject("myfriend", "승호");
    ...
    // 요청 URI 접수
    // 1. 컨트롤러는 골뱅이Controller 내부에 골뱅에 GetMapping 어노테이션을 통해
    // 클라이언트의 요청을 받음
    @GetMapping("/bye")
    public String seeYouNext(Model model) { // 2. 메서드 작성 : /bye 요청을 처리
        // 3. goodbye.mustache 변환
        // 반환값은 요청에 따라 보여줄 뷰 템플릿 페이지를 적용
        // /template/goodbye.mustache를 포워딩
        model.addAttribute("nickname", "석현");
        return "goodbye";
    }
    ...
    */
    ...
    요청 파라미터 : request{title=개똥이의 모험, content = 즐거운 모험}
    요청 방식 : post
     */
    ...
    public String createArticle(ArticleForm form) {
        ...
        /*Long listCnt = this.articleRepository.count() + 1;
        form.setId(listCnt);*/
        ...
        // JPA vs MyBATIS의 경우 return type은 int 타입 반환
        Article saved = this.articleRepository.save(article);
        log.info("createArticle -> saved : " + saved);
    ...
    public String show(@PathVariable(value="id") Long id, Model model) { // 매개변수로 id 받아오기
     ...
     */
    @GetMapping("/articles")
    public String index(Model model) { // index() 메서드의 매개변수로 model 객체를 받아옴
        //1. 모든 데이터 가져오기
        //findAll() 메서드의 반환 데이터 타입은 Iterable. List라서 불일치
        //첫째, 캐스팅(형변환). Iterable, Collection, List 인터페이스의 상하 관계는
    ...
    public String editForm(ArticleForm form) {
    ...
    public String delete(@PathVariable(value="id") Long id, RedirectAttributes rttr) {
        ...
        Article articleEntity = this.articleRepository.findById(id).orElse(null);
        log.info("삭제 대상 articleEntity : " + articleEntity);
        ...
        // 이해비법 : redirect 시 사용되는 model(forwarding 시에 사용)이라고 생각해도 된다.
        rttr.addFlashAttribute("msg", "삭제되었습니다.");

        //3) 결과 페이지로 리다이렉트하기
        // 글 목록 페이지(/articles)로 리다이렉트합니다.
